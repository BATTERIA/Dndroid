...
import com.bilibili.bililive.batteria.recycler.SKViewHolder
import com.bilibili.bililive.batteria.recycler.SKViewHolderFactory
...
open class SKRecyclerViewAdapter<T : Any> : RecyclerView.Adapter<SKViewHolder<*>>() {
    ...
    @CallSuper
    @Suppress("UNCHECKED_CAST")
    override fun onBindViewHolder(holder: SKViewHolder<*>, position: Int, payloads: MutableList<Any>) {
        if (payloads.isEmpty()) {
            this.onBindViewHolder(holder, position);
        } else {
            val item = getItem(position) ?: return
            (holder as? SKViewHolder<T>)?.bind(item, payloads)
        }
    }
...
class AdapterDataWrapper<T : Any> {
    ...
    fun indexOfLast(predicate: (T) -> Boolean): Int {
        return dataList.indexOfLast(predicate)
    }
