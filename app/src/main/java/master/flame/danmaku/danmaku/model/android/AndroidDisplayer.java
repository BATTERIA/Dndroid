...
public class AndroidDisplayer extends AbsDisplayer<Canvas, Typeface> {
    ...
    public static class DisplayerConfig {
         ...
         * 字体顶部间距缩短百分比
         */
        public float FONT_SPACING_MULT = 0f;
        ...
        public boolean CONFIG_HAS_FOUR_DIR_SHADOW = false;
        public boolean HAS_FOUR_DIR_SHADOW = CONFIG_HAS_FOUR_DIR_SHADOW;
        ...
        public void setTypeface(Typeface typeface, float fontSpacingMult) {
            this.PAINT.setTypeface(typeface);
            this.FONT_SPACING_MULT = fontSpacingMult;
        }
        ...
        public void definePaintParams(boolean fromWorkerThread) {
            ...
            HAS_FOUR_DIR_SHADOW = CONFIG_HAS_FOUR_DIR_SHADOW;
            HAS_PROJECTION = CONFIG_HAS_PROJECTION;
            ANTI_ALIAS = CONFIG_ANTI_ALIAS;
        }
    }

    private Map<Integer, Integer[]> mVirtualMarginsMap = new ConcurrentHashMap<>();
    ...
    public void setTypeFace(Typeface font, float fontSpacingMult) {
        mDisplayConfig.setTypeface(font, fontSpacingMult);
    }
    ...
    @Override
    public void setVirtualMargins(int type, int left, int top, int right, int bottom) {
        mVirtualMarginsMap.put(type, new Integer[]{left, top, right, bottom});
    }

    @Override
    public void resetVirtualMargins(int type) {
        mVirtualMarginsMap.remove(type);
    }
...
//            height = c.getHeight();
    ...
    public int draw(BaseDanmaku danmaku) {
        ...
        if (canvas != null) {
            ...
            if (mDisplayConfig.isTranslucent || danmaku.isPlayingBlockAnimation()) {
                alphaPaint = mDisplayConfig.ALPHA_PAINT;
                alphaPaint.setAlpha(mDisplayConfig.transparency);
            }
            if (danmaku.getType() == BaseDanmaku.TYPE_SPECIAL) {
    ...
    public synchronized void drawDanmaku(BaseDanmaku danmaku, Canvas canvas, float left, float top,
        ...
        if (sStuffer != null) {
            sStuffer.drawDanmaku(this, danmaku, canvas, left, top, fromWorkerThread, mDisplayConfig);
        }
    }
    ...
    private void calcPaintWH(BaseDanmaku danmaku, TextPaint paint, boolean fromWorkerThread) {
        sStuffer.measure(this, danmaku, paint, fromWorkerThread);
    ...
    public void setDanmakuStyle(int style, float[] values) {
        switch (style) {
            case DANMAKU_STYLE_NONE:
                ...
                mDisplayConfig.CONFIG_HAS_FOUR_DIR_SHADOW = false;
            ...
            case DANMAKU_STYLE_PROJECTION:
                ...
                break;
            case DANMAKU_STYLE_FOUR_DIR_SHADOW:
                mDisplayConfig.CONFIG_HAS_SHADOW = true;
                mDisplayConfig.CONFIG_HAS_FOUR_DIR_SHADOW = true;
                mDisplayConfig.CONFIG_HAS_STROKE = false;
                mDisplayConfig.CONFIG_HAS_PROJECTION = false;
                setShadowRadius(values[0]);
                break;
    ...
    @Override
    public boolean isHardwareBimtapEnabled() {
        return false;
    }
    ...
    @Override
    public Integer[] getVirtualMargins(int type) {
        return mVirtualMarginsMap.get(type);
    }

    @Override
    public int getVirtualMargin(int type, int direction) {
        Integer[] virtualMargins = getVirtualMargins(type);
        if (virtualMargins != null) {
            return virtualMargins[direction];
        }
        return 0;
    }

    @Override
    public int getVirtualWidth(int type) {
        int marginLeft = getVirtualMargin(type, MARGIN_LEFT);
        int marginRight = getVirtualMargin(type, MARGIN_RIGHT);
        return getWidth() - marginLeft - marginRight;
    }

    @Override
    public float getFontSpacingMult() {
        return mDisplayConfig.FONT_SPACING_MULT;
    }
