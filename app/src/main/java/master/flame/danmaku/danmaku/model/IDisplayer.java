...
public interface IDisplayer {
    ...
    int DANMAKU_STYLE_FOUR_DIR_SHADOW = 4; //四向阴影
    ...
    boolean isHardwareBimtapEnabled();

    int getMaximumCacheWidth();

    int getMaximumCacheHeight();

    Integer[] getVirtualMargins(int type);

    int getVirtualMargin(int type, int direction);

    int getVirtualWidth(int type);

    float getFontSpacingMult();
    ...
    void setScreenOccupationRatio(float screenOccupationRatio);
    ...
    void setVirtualMargins(int type, int left, int top, int right, int bottom);

    void resetVirtualMargins(int type);
