...
public class SpecialDanmaku extends BaseDanmaku {
    ...
    public void layout(IDisplayer displayer, float x, float y) {
        getRectAtTime(displayer, mTimer.current());
    }
...
//        float b = 0f;
    ...
    public void copySpecialDanmaku(SpecialDanmaku target, float scaleX, float scaleY) {
        if (target == null) {
            return;
        }
        beginX = target.beginX * scaleX;
        beginY = target.beginY * scaleY;
        endX = target.endX * scaleX;
        endY = target.endY * scaleY;
        deltaX = endX - beginX;
        deltaY = endY - beginY;
        ...
        beginAlpha = target.beginAlpha;
        endAlpha = target.endAlpha;
        deltaAlpha = endAlpha - beginAlpha;
        alphaDuration = target.alphaDuration;
        if (beginAlpha != AlphaValue.MAX) {
            alpha = beginAlpha;
        }
        translationDuration = target.translationDuration;
        translationStartDelay = target.translationStartDelay;
        isQuadraticEaseOut = target.isQuadraticEaseOut;
        mScaleFactorChangedFlag = target.mScaleFactorChangedFlag;
        if (target.linePaths != null) {
            int length = target.linePaths.length;
            linePaths = new LinePath[length];
            for (int i = 0; i < length; i++) {
                LinePath line = target.linePaths[i];
                linePaths[i] = new LinePath();
                linePaths[i].setPoints(new Point(line.pBegin.x * scaleX, line.pBegin.y * scaleY),
                        new Point(line.pEnd.x * scaleX, line.pEnd.y * scaleY));
            }
            float totalDistance = 0;
            for (LinePath line : linePaths) {
                totalDistance += line.getDistance();
