...
public class Danmakus implements IDanmakus {
    ...
    public Danmakus(int sortType, boolean duplicateMergingEnabled, BaseComparator baseComparator) {
        ...
        } else if (sortType == ST_BY_WEIGHT) {
            comparator = new WeightComparator(duplicateMergingEnabled);
        }
    ...
    @Override
    public IDanmakus subNewSet(long startTime, long endTime) {
        synchronized (mLockObject) {
            Collection<BaseDanmaku> subset = subset(startTime, endTime);
            if (subset == null || subset.isEmpty()) {
                return new Danmakus();
            }
            TreeSet<BaseDanmaku> newSet = new TreeSet<>(new TimeComparator(false));
    ...
    @Override
    public BaseDanmaku get(IDanmakus.Checker checker) {
        synchronized (this.mLockObject) {
            Iterator<BaseDanmaku> it = items.iterator();
            while (it.hasNext()) {
