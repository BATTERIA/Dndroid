...
public class R2LDanmaku extends BaseDanmaku {
    ...
    public void layout(IDisplayer displayer, float x, float y) {
        if (mTimer != null) {
            long currMS = mTimer.current();
            this.x = isHovered ? getAccurateLeft(displayer, mHoverTime) : getAccurateLeft(displayer, currMS - mHoverTime);
            if (!this.isShown()) {
                setY(displayer, y);
            }
            mLastTime = currMS;
            long deltaDuration = currMS - getActualTime();
            if (isHovered || deltaDuration > 0 && deltaDuration < duration.value + mHoverTime) {
...
//            return displayer.getWidth() + paintWidth;
//        }
    ...
    public float getRight() {
        return getLeft() + paintWidth;
    }

    @Override
    public float getBottom() {
        return getTop() + paintHeight;
    }
