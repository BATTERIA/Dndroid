...
public class CacheManagingDrawTask extends DrawTask {
    ...
    protected void initTimer(DanmakuTimer timer) {
        ...
        mCacheTimer.update(timer.current());
    }
    ...
    public long getCurrentCachingTimeMills() {
        if (mCacheManager != null) {
            return mCacheManager.getCurrentCachingTimeMills();
        }
        return super.getCurrentCachingTimeMills();
    }

    @Override
    public long getCurrentCachingBeginTimeMills() {
        if (mCacheManager != null) {
            return mCacheManager.getCurrentCachingBeginTimeMills();
        }
        return super.getCurrentCachingBeginTimeMills();
    }
    ...
    public RenderingState draw(AbsDisplayer displayer, DrawHelper.Mode mode) {
        RenderingState result = super.draw(displayer, mode);
    ...
    public void seek(long mills) {
        super.seek(mills);
        if (mContext.cacheMissFilter != null) {
            mContext.unregisterFilter(mContext.cacheMissFilter);
        }
        if (mCacheManager == null) {
            start();
        }
    ...
    public class CacheManager implements ICacheManager {
        ...
        Pool<DrawingCache> mCachePool = Pools.finitePool(mCachePoolManager, getMaxCapacity());
        ...
        public void begin() {
            mEndFlag = false;
            if (mThread == null) {
                mThread = new HandlerThread("DFM Cache-Building Thread", Thread.MIN_PRIORITY);
                mThread.start();
            }
            if (mHandler == null)
...
//Log.i("DFM CACHE", "realsize:"+mRealSize + ",size" + size);
        ...
        public long getCurrentCachingBeginTimeMills() {
            ...
            BaseDanmaku first = mCaches.first();
            if (first == null) {
                return 0;
            }
...
//Log.e(TAG,"dispatch_actions:"+mCacheTimer.currMillisecond+":"+mTimer.currMillisecond);
                    ...
                    case BUILD_CACHES:
                        ...
                        Long expiredBeginTime = (Long) msg.obj;
                        if (expiredBeginTime != null) {
                            mCacheTimer.update(expiredBeginTime);
                        }
...
//                        Log.i(TAG,"BUILD_CACHES:"+mCacheTimer.currMillisecond+":"+mTimer.currMillisecond);
                    ...
                    case SEEK:
                        ...
                        if (seekMills != null) {
                            ...
                            long oldCacheTime = mCacheTimer.current();
                            ...
                            resume();
                            if (mContext.cacheMissFilter != null) {
                                mContext.registerFilter(mContext.cacheMissFilter);
                            }
                        }
                        break;
                    case QUIT:
                    ...
                    case CLEAR_ALL_CACHES:
                        evictAll();
                        mCacheTimer.update(mTimer.current() - mContext.mDanmakuFactory.MAX_DANMAKU_DURATION);
                        mSeekedFlag = true;
                        break;
                    case CLEAR_OUTSIDE_CACHES:
                        evictAllNotInScreen();
                        mCacheTimer.update(mTimer.current());
                        break;
            ...
            private long dispatchAction() {
                if (mCacheTimer.current() <= mTimer.current() - mContext.mDanmakuFactory.MAX_DANMAKU_DURATION) {
                ...
                long gapTime = firstCache != null ? firstCache.getActualTime() - mTimer.current() : 0;
                ...
                // check cache time
                long deltaTime = mCacheTimer.current() - mTimer.current();
                ...
                danmakus.forEach(new IDanmakus.DefaultConsumer<BaseDanmaku>() {
                    ...
                    public int accept(BaseDanmaku item) {
                        ...
                        if (item.isTimeOut()) {
                            ...
                            if (cache != null && cache.get() != null) {
                                entryRemoved(true, item, null);
                            }
                            return ACTION_CONTINUE;
                        }
                        if (item.priority == 0 && !item.hasPassedFilter()) {
...
//                String message = "";
                ...
                danmakus.forEach(new IDanmakus.DefaultConsumer<BaseDanmaku>() {
                    ...
                    public int accept(BaseDanmaku item) {
                        ...
                        if (item.isTimeOut() || item.isFiltered()) {
                            return ACTION_CONTINUE;
                        }

                        if (last.getActualTime() < mTimer.current()) {
                            return ACTION_BREAK;
                        }
...
//Log.e("prepareCache", currScreenIndex+","+indexInScreen+"," + item.time+"skip:"+skip);
                        ...
                        // build cache
                        if (RESULT_FAILED == buildCache(item, false)) {
                            return ACTION_CONTINUE;
                        }
                        if (!repositioned) {
...
//Log.w("cache", danmaku.text + "DrawingCache hit!!:" + item.paintWidth + "," + danmaku.paintWidth);
                    ...
                    if (!mDisp.isHardwareBimtapEnabled()) {  // 使用hardware bitmap时不进行bitmap冗余复用
...
//Log.e("cache", "break at exception:" + e.getMessage());
            ...
            private final void addDanmakuAndBuildCache(BaseDanmaku danmaku) {
                if (danmaku.isTimeOut() || (danmaku.getActualTime() > mCacheTimer.current() + mContext.mDanmakuFactory.MAX_DANMAKU_DURATION && !danmaku.isLive)) {
                    return;
                }
            ...
            public void requestBuildCacheAndDraw(long correctionTime) {
                ...
                mSeekedFlag = true;
                sendEmptyMessage(DISABLE_CANCEL_FLAG);
                obtainMessage(CacheHandler.BUILD_CACHES, mTimer.current() + correctionTime).sendToTarget();
            }
